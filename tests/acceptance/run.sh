...
if ! docker image inspect "$ioc_image" >/dev/null 2>&1; then
  fail "Docker image '$ioc_image' is not loaded; load the acceptance IOC image before running this script"
fi
...
"$repo_root/tests/acceptance/phoebus/prepare-mock-ioc.sh" \
  --docker-container "$ioc_name" \
  | tee "$evidence_dir/mock-ioc-preparation.log"

"$repo_root/tests/acceptance/phoebus/bootstrap.sh"
"$repo_root/tests/acceptance/phoebus/self-test.sh"
...
export ELI_LAUNCHER_CONFIG="$repo_root/tests/acceptance/config/launcher.yaml"

npm run build \
  | tee "$evidence_dir/build.txt"
"$repo_root/node_modules/.bin/tsx" "$repo_root/scripts/validate-config.ts" \
  "$repo_root/tests/acceptance/config/launcher.yaml" \
...
timeout 180 "$repo_root/node_modules/.bin/tsx" \
  "$repo_root/tests/acceptance/drive-electron.ts" \
...
memento_file="$repo_root/tests/acceptance/assets/local-alarm-layout.memento"
...
setsid "$repo_root/tests/acceptance/phoebus/phoebus.sh" \
  -server "$phoebus_port" \
...
jq -n \
  ...
  '{
    ...
    electron: "six production launch IPC calls passed and runtime state converged",
    ...
    siteClaim: "no site LabVIEW, alarm backend, or deployment value was asserted"
