...
test("`hmi-server` is kept as a named host, not wired to hmiApi.baseUrl", () => {
  // src/main/hmi-api.ts rejects a non-loopback baseUrl that is not HTTPS, and the
  // real value has no scheme at all, so auto-wiring would break every host config.
  const { local } = map(REAL_HOST);
  assert.equal((local["hmiApi"] as unknown), undefined);
