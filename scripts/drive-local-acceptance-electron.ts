...
import { readLabviewContractCapture } from "./labview-contract-capture";
...
async function waitForRuntime(client: CdpClient): Promise<RuntimeSnapshot> {
  ...
  while (Date.now() < deadline) {
    ...
    if ([...expected].every(([id, status]) => byId.get(id) === status)) {
      assert.equal(latest.hmiApi?.status, "connected");
      return latest;
    }
    await delay(100);
  }
...
async function waitForLifecycleEntries(expectedIds: string[]): Promise<unknown> {
  const baseUrl =
  ...
  while (Date.now() < deadline) {
    ...
    const entries = (latest as { entries?: Array<{ report?: { entryId?: string } }> }).entries ?? [];
    const ids = new Set(entries.map((entry) => entry.report?.entryId));
    if (expectedIds.every((id) => ids.has(id))) {
      return latest;
    }
    await delay(100);
  }
  throw new Error(`Lifecycle service did not receive every launcher entry: ${JSON.stringify(latest)}`);
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const expectedPhoebusTitles = new Map<string, string[]>([
      ...
      ["local-phoebus-temperature", ["CSI-744 Local Temperature"]],
      ["local-phoebus-state", ["CSI-744 Local State"]],
      ["local-phoebus-flow", ["CSI-744 Local Chiller"]],
    ]);
    ...
    const lifecycle = await waitForLifecycleEntries(launchIds);
    ...
    // Closing the only renderer exercises Electron's before-quit lifecycle
    // deregistration path instead of killing the process immediately.
    try {
