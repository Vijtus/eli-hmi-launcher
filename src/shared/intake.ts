// Deterministic conversion of the L4 GUI intake sheet (CSV) into launcher YAML.
//
...
export function convertIntakeCsv(csvText: string): IntakeConversion {
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    lines.push(`    technology: ${technology.length > 0 ? yamlScalar(technology.join("; ")) : "--"}`);
    lines.push(`    section: ${section.length > 0 ? yamlScalar(section.join("; ")) : "--"}`);
  ...
  const header = [
    "# Generated from the L4 GUI intake sheet by scripts/intake-to-yaml.ts.",
