// FR1 / NFR1-NFR4, NFR7 — obtain the configuration repository at startup.
//
...
// so the token can be held in memory only (see config-repo-auth.ts). No child
// process is spawned, so the token cannot appear in a process table.
//
// Update strategy (NFR2): clone when the cache is absent, otherwise fetch and
...
//
// Offline resilience (NFR3): a network failure with a usable cache is a warning,
...
import { createAuthCallback, redactError, redactSecret, type GitAuthCredentials } from "./config-repo-auth";
...
// NFR7 — restrictive permissions. `mode` is a no-op on Windows, which is fine:
