...
$ npm run validate-config -- config/launcher.example-real.yaml
OK  config/launcher.example-real.yaml
...
| CSI-846 — local machine configuration | **IMPLEMENTED** | `config/local-acceptance.yaml` joined workspace, CSS root, Phoebus, lifecycle, monitoring, and both command allow-list roots. The real parser reported 5 rows, 1 action, 6 unique ids, and 2 restricted launches. |
...
| CSI-851 — navigation UI | **IMPLEMENTED** | The combined launcher rendered five rows with the seven expected columns; both fixtures showed `RUNNING` and all three BOB rows showed `SHARED`. `reference-screenshots/07-local-acceptance-launcher.png` records the state. |
...
- `reference-screenshots/05-local-phoebus-live.png`
- `reference-screenshots/06-local-alarm-layout-restored.png`
- `reference-screenshots/07-local-acceptance-launcher.png`
...
$ PYTHONPATH=<handoff FastAPI site-packages> /usr/bin/python3 -m pytest services/hmi-lifecycle-api/tests
14 passed, 1 deprecation warning
[exit status: 0]

$ npm run validate-config -- config/phoebus-local.yaml
3 entries; 1 action; 4 unique ids
[exit status: 0]

$ npm run validate-config -- config/local-acceptance.yaml
...
$ PYTHONPATH=<handoff venv site-packages> python3 -m pytest services/hmi-lifecycle-api/tests
14 passed, 1 deprecation warning
[exit status: 0]
