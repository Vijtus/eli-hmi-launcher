# Local HMI lifecycle contract

This loopback service is an executable **local launcher-lifecycle contract**.
...
```sh
python -m venv .venv
.venv/bin/pip install -r services/hmi-lifecycle-api/requirements.txt
.venv/bin/python services/hmi-lifecycle-api/lifecycle_api.py
```
...
```sh
python -m pytest services/hmi-lifecycle-api/tests
```
